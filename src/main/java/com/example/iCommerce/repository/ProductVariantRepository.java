...
public interface ProductVariantRepository extends JpaRepository<ProductVariant, String> {
    ...
    @Query(value = """
        ...
        ORDER BY\s
            pv.create_day DESC
       \s""", nativeQuery = true)
    List<Object[]> findProductVariants();
    ...
    @Query(value = """
        SELECT\s
            ...
            pv.stop_day AS stop_day,
            MAX(CASE\s
                WHEN d.end_day > CURRENT_TIMESTAMP\s
                THEN d.percent\s
            END) AS percent,
            MIN(CASE\s
                WHEN d.end_day > CURRENT_TIMESTAMP\s
        ...
        ORDER BY\s
            pv.create_day DESC
       \s""", nativeQuery = true)
    List<Object[]> findProductVariantsByProductIdAdmin(@Param("productId") String productId);
    ...
    SELECT
        ...
        pv.stop_day AS stop_day,
        MAX(CASE
            WHEN d.end_day > CURRENT_TIMESTAMP
            THEN d.percent
        END) AS percent,
        MIN(CASE
            WHEN d.end_day > CURRENT_TIMESTAMP
    ...
    FROM
        ...
        LEFT JOIN discount d ON d.product_variant_id = pv.id
            AND d.end_day > CURRENT_TIMESTAMP
        LEFT JOIN variant_attribute va ON va.product_variant_id = pv.id
        LEFT JOIN attribute_value av ON av.id = va.attribute_value_id
        LEFT JOIN attribute a ON a.id = av.attribute_id
        LEFT JOIN rating r ON r.product_variant_id = pv.id
    ...
    LIMIT 10
    """, nativeQuery = true)
    List<Object[]> findLowestStockProductVariants();
